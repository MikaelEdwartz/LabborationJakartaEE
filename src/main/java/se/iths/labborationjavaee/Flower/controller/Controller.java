...
import se.iths.labborationjavaee.Flower.mapper.Mapper;
import se.iths.labborationjavaee.Flower.repository.Repository;
...
public class Controller {
    ...
    public Response updateFlower(@PathParam("id") Long id, @QueryParam("name") String name, @QueryParam("color") String color) {
        checkIfValidInput(id, name, color);
        callRepositoryAndChangeValue(id, name, color);

        return Response.ok().entity(mapper.map(repository.findById(id).get())).build();
    }

    private void checkIfValidInput(Long id, String name, String color) {
    ...
    private void callRepositoryAndChangeValue(Long id, String name, String color) {
        if (color == null)
